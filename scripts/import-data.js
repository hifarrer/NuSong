...
// Function to fix malformed features JSON
function fixFeatures(features) {
  if (typeof features === 'string') {
    try {
      // Remove trailing comma and parse
      const cleanedFeatures = features.replace(/,\s*]/g, ']').replace(/,\s*}/g, '}');
      return JSON.parse(cleanedFeatures);
    } catch (error) {
      console.warn('Could not parse features string, using empty array:', error.message);
      return [];
    }
  }
  return features;
...
async function importSubscriptionPlans() {
  ...
  for (const plan of plans) {
    try {
      ...
      const fixedFeatures = fixFeatures(plan.features);
